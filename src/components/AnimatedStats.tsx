...
export default function AnimatedStats({ value, label, suffix = '', prefix = '', icon: Icon, color = 'blue' }: Props) {
  ...
  return (
    <div className={cn('bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 transition-opacity duration-300', visible ? 'opacity-100' : 'opacity-0')}>
      {Icon && (
        <div className={cn('w-12 h-12 rounded-xl flex items-center justify-center mb-4 bg-gradient-to-br', colors[color])}>
