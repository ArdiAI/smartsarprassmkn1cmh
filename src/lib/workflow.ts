...
export interface WorkflowStep {
  id: string; workflow_template_id: string; step_order: number;
  role_id: string | null; step_label: string; is_info_only: boolean; role_name?: string;
}
export interface WorkflowTemplate {
  id: string; name: string; description: string; is_active: boolean; steps: WorkflowStep[];
}
...
export function getCurrentStep(steps: WorkflowStep[], currentStep: number): WorkflowStep | null {
  if (currentStep === 0) return steps.find(s => !s.is_info_only) || null;
...
export function isLastActionableStep(steps: WorkflowStep[], currentStep: number): boolean {
  const actionable = steps.filter(s => !s.is_info_only);
  return actionable[actionable.length - 1]?.step_order === currentStep;
}
