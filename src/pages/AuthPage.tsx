        ...
        <div className="max-w-md w-full">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 border border-slate-200 dark:border-slate-700 text-center">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">
              Kami telah mengirim email konfirmasi ke <strong>{email}</strong>. Silakan cek email Anda dan klik link konfirmasi untuk mengaktifkan akun.
            </p>
          ...
          <div className="flex gap-1 mb-6 bg-slate-100 dark:bg-slate-700 p-1 rounded-xl">
            {(['login', 'register'] as const).map(m => (
              <button key={m} onClick={() => { setMode(m); setError(''); }}
                className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${mode === m ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-slate-400'}`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} placeholder="Minimal 6 karakter"
                  className="w-full pl-10 pr-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            <button type="submit" disabled={loading}
              ...
              {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <>{mode === 'login' ? 'Masuk' : 'Daftar'} <ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>

          {mode === 'register' && (
            <p className="text-xs text-slate-400 text-center mt-4">
              Dengan mendaftar, Anda akan menerima email konfirmasi untuk mengaktifkan akun.
            </p>
          )}
        </div>

        <p className="text-center text-sm text-slate-500 mt-4">
          <Link to="/" className="hover:text-blue-500">Kembali ke Beranda</Link>
        </p>
      </div>
    </div>
  );
